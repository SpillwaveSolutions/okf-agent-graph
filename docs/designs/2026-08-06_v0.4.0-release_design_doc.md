---
wiki_key: design/2026-08-06_v0.4.0-release-design-doc
doc_type: design
...
| Safe multi-agent contracts | Schemas, loop controls, append outputs, guarded irreversible tools, and checks on secret-like scalar frontmatter keys | `scripts/ager-validate.py` — `validate()`, lines 152–210 |
...
sequenceDiagram
    ...
    participant Init as ⚙️ ager-init.py
    participant Stage as 📦 Temporary bundle
    participant Validator as ✅ ager-validate.py
    participant OKF as 🔗 okf-graph-eng

    User->>Init: create destination and title
    Init->>Stage: copy scaffold and render placeholders
    Init->>Validator: strict validation
    alt AGER invalid
        Validator-->>Init: JSON issues and nonzero exit
        Init-->>User: refuse install and clean up
    else AGER valid
        Validator-->>Init: valid JSON result
        Init->>Init: atomically install destination
        Init-->>User: bundle summary
        User->>OKF: strict graph validation
        OKF-->>User: graph result
    end
```

Purpose: show execution order and the deliberate boundary between initialization
and external OKF validation. The initializer never installs an AGER-invalid
staging tree; OKF validation remains a separate user/CI step.
...
them to the bundle and checks referenced JSON syntax (not JSON Schema
semantics); `validate()` applies concept rules and
...
## 22. Security and resilience

The validator checks non-list frontmatter lines whose keys look secret-like and
rejects literal values unless they are path or environment references. It does
not scan Markdown bodies, JSON/text files, or list-item mappings for secrets
(`scripts/ager-validate.py` — `validate()`, lines 200–207). It also contains
...
successful generation, and overwrite refusal (lines 31–129). Packaging tests
...
## 31. Risks and extension roadmap
...
| Requirement | Workflow / component | Tests |
|---|---|---|
| Reproducible bundle creation | Create/validate workflow; generator | `AgerInitTests` |
| AGER contract enforcement | Validator rules and reference resolution | `AgerValidationTests` |
| Full OKF graph integrity | External OKF validation boundary | `test_sample_passes_strict_okf_validation` |
| Cross-host packaging | Host manifests and shared skills | `PluginPackagingTests` |
...
## 35. Appendices and omitted sections
