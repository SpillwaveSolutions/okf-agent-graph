...
def test_isolation_two_sessions_do_not_clobber():
    with tempfile.TemporaryDirectory() as td:
        ...
        (Path(sa["bundle"]) / "agents").mkdir(exist_ok=True)
        (Path(sb["bundle"]) / "agents").mkdir(exist_ok=True)
        (Path(sa["bundle"]) / "agents" / "lumenfield-orchestrator.md").write_text(
            "---\ntype: AgentNode\ntitle: Lumenfield Orchestrator\n---\n", encoding="utf-8"
        )
        (Path(sb["bundle"]) / "agents" / "northstar-judge.md").write_text(
            "---\ntype: AgentNode\ntitle: Northstar Judge\n---\n", encoding="utf-8"
        )

        assert (Path(sa["bundle"]) / "agents" / "lumenfield-orchestrator.md").exists()
        assert not (Path(sa["bundle"]) / "agents" / "northstar-judge.md").exists()
        assert (Path(sb["bundle"]) / "agents" / "northstar-judge.md").exists()
        assert not (Path(sb["bundle"]) / "agents" / "lumenfield-orchestrator.md").exists()
        ...
        assert (knowledge / "agents" / "lumenfield-orchestrator.md").exists()
        assert (knowledge / "agents" / "northstar-judge.md").exists()
